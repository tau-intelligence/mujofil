"""Test the user's hypothesis: render_batch wastes per-view compute at low res on
effects you don't need (MSAA 4x supersample-resolve, SSAO, SSR, shadows -- each a
per-view pass run N times). Strip them for the low-res/high-throughput regime
while KEEPING gltfio full PBR + FILMIC tonemap (so it stays photoreal, NOT flat
like layered). Measure cam/s per config + save a quality montage of the fast one.
"""
import os, sys, time
...
# (label, kwargs applied to RendererConfig). All keep gltfio PBR + post/tonemap.
CONFIGS = [
    ("full(default)", dict(enable_msaa=True,  enable_ssao=True,  enable_ssr=True,  enable_shadows=True)),
    ("no-MSAA",       dict(enable_msaa=False, enable_ssao=True,  enable_ssr=True,  enable_shadows=True)),
    ("no-SSAO/SSR",   dict(enable_msaa=False, enable_ssao=False, enable_ssr=False, enable_shadows=True)),
    ("minimal(RL)",   dict(enable_msaa=False, enable_ssao=False, enable_ssr=False, enable_shadows=False)),
]
...
def bench(scene, res, n, label, kw, save=False):
    ...
    if save:
        ...
        out = os.path.join(HERE, "out", "batch_fast"); os.makedirs(out, exist_ok=True)
        Image.fromarray(montage(img, res)).save(os.path.join(out, f"{scene}_{label.split('(')[0]}_N{n}.png"))
    r.close()
    return cps
...
def main():
    ...
    print(f"render_batch effect-strip, res={res} N={n}  (cam/s; all keep full gltfio PBR + FILMIC)\n")
    for scene in scenes:
        print(f"--- {scene} ---")
        base = None
        for label, kw in CONFIGS:
            cps = bench(scene, res, n, label, kw, save=(label == "minimal(RL)"))
            if base is None: base = cps
            print(f"  {label:>16}: {cps:>7.0f} cam/s   ({cps/base:.2f}x vs full)")
        print()
